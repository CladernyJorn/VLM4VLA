...
class BaseRoboVLM(nn.Module):
    ...
    def forward_continuous(
        ...
        **kwargs,
        ...
        # Prepare layer-wise VL embeddings list for FMDecoder if needed
        vl_embs_list = None
        if self.act_head is not None and hasattr(self.act_head, 'model') and hasattr(
                self.act_head.model, 'transformer_blocks'):
            # Check if this is FMDecoder by checking for transformer_blocks attribute
            expected_layers = len(self.act_head.model.transformer_blocks)
            all_hidden_states = output.hidden_states

            # Reshape hidden states if needed (similar to output_hs reshape)
            vl_embs_list = []
            for hidden_state in all_hidden_states:
                ...
                if history_type == "pre":
                    ...
                    # But we may need to reshape if it's (bs, seq_len*seq_length, hidden_dim)
                    if hidden_state_clone.shape[0] == bs:
                        # Reshape from (bs, seq_len*seq_length, hidden_dim) to (bs*seq_len, seq_length, hidden_dim)
                        # This is approximate - we assume tokens are distributed evenly
                        ...
                        hidden_state_clone = hidden_state_clone.reshape(bs * seq_len, -1, hidden_state_clone.shape[-1])
                vl_embs_list.append(hidden_state_clone)

        # Call action head with vl_embs_list if available
